...
class TestPerAgentAuditTrails:
    def test_log_agent_event(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            entries = [json.loads(l) for l in trail_path.read_text().strip().split("\n")]
...
class TestAgentPermissions:
    def _write_policy(self, tmpdir, agent_perms):
        ...
        config_dir.joinpath("config.yaml").write_text(yaml.dump({
            "multi_agent": {
...
class TestDoctorCommand:
    ...
    def test_doctor_valid_setup(self, capsys):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            trib = Path(tmpdir) / ".tribunal"
            trib.mkdir()
            (trib / "rules.yaml").write_text(yaml.dump({"rules": {"test": {"trigger": "PreToolUse", "action": "block"}}}))
            claude = Path(tmpdir) / ".claude"
            claude.mkdir()
            (claude / "claudeconfig.json").write_text(json.dumps({"hooks": {"PreToolUse": [{"run": [{"command": "tribunal-gate"}]}]}}))
...
class TestConfigValidateCLI:
    ...
    def test_config_validate_valid(self, capsys):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            trib = Path(tmpdir) / ".tribunal"
            trib.mkdir()
            (trib / "config.yaml").write_text(yaml.dump({"budget": {"session_usd": 2.0}}))
            with patch("tribunal.cli.Path.cwd", return_value=Path(tmpdir)):
                result = cmd_config(argparse.Namespace(config_command="validate"))
            assert result == 0
            captured = capsys.readouterr()
