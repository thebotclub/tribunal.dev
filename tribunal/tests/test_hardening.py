...
class TestConfigValidation:
    ...
    def test_rules_invalid_action(self):
        ...
        errors = validate_config(
            {"rules": {"bad-rule": {"action": "explode", "trigger": "PreToolUse"}}}
        )
    ...
    def test_rules_unknown_trigger(self):
        ...
        errors = validate_config(
            {"rules": {"bad-trigger": {"trigger": "OnMoonPhase", "action": "block"}}}
        )
...
class TestMemoryLimits:
    ...
    def test_inject_memory_basic(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            entry = MemoryEntry(
                title="Test Rule", content="Do TDD", memory_type="pattern"
            )
...
class TestAnalytics:
    ...
    def test_analyze_costs_to_dict(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            self._setup_state(
                tmpdir, {"2026-04-01": 1.0, "2026-04-02": 2.0, "2026-04-03": 3.0}
            )
            analytics = analyze_costs(tmpdir)
            d = analytics.to_dict()
    ...
    def test_export_and_import_bundle(self):
        ...
        with tempfile.TemporaryDirectory() as src, tempfile.TemporaryDirectory() as dst:
            ...
            (rules_dir / "config.yaml").write_text(
                yaml.dump({"budget": {"session_usd": 5}})
            )
...
class TestDashboard:
    ...
    def test_load_audit_events(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            audit_dir.mkdir()
            entries = [
                json.dumps({"tool": "FileEdit", "allowed": True}) for _ in range(5)
            ]
    ...
    def test_generate_html_report(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            audit_dir.mkdir()
            entries = [
                json.dumps({"hook": "PreToolUse", "tool": "FileEdit", "allowed": True})
            ]
            (audit_dir / "audit.jsonl").write_text("\n".join(entries))
...
class TestMissingToolDetection:
    ...
    def test_type_check_missing_tsc_warns(self, capsys):
        from tribunal.rules import _condition_type_check, Rule
        from tribunal.protocol import HookEvent
